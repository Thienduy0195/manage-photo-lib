...
const localDataName = 'mateGallery';
...
const fakedata = [
  ...
  },
  {
    ...
    desc: 'Réhahn Photography was showcased in major news outlets around the world in 2018 and 2019. Articles in the BBC, New York Times, South China Morning Gallery and more include photographs from projects such as Hidden Smile and Precious Heritage among others.',
    img: Img3,
  },
  {
    id: 'd',
...
const createDemoData = async (page = 1) => {
  const res = await httpRequest.get(`galleries?page=${page}`)
  return res.data
};

const removeGallery = async (id) => {
  const res = await httpRequest.delete(`galleries/${id}`);
  return res.status
}
